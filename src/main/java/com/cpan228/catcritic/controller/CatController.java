...
public class CatController {
    ...
    public String submitCat(@Valid @ModelAttribute("cat") Cat cat, BindingResult result,
        ...
        try {
            Cat saved = catService.saveCat(cat, photo, currentUser.getUsername());
    ...
    @GetMapping("/cats/{id}")
    public String showCat(@PathVariable Long id, Model model, @AuthenticationPrincipal User currentUser) {
        Cat cat = catService.getCatById(id);
        if (cat == null) {
            return "redirect:/cats";
        }

        Rating userRating = ratingService.getUserRatingForCat(cat.getId(), currentUser.getUsername());
    ...
    public String browse(
        ...
        for (Cat cat : cats) {
            Rating rating = ratingService.getUserRatingForCat(cat.getId(), currentUser.getUsername());
            if (rating != null) {
    ...
    public String rateCat(@PathVariable Long id, @RequestParam int stars,
        ...
        if (cat != null && stars >= 1 && stars <= 10) {
            ratingService.rateCat(cat, stars, currentUser.getUsername());
        }
